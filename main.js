...
const getPower = async (headers, proxy) => {
    ...
    log.debug(`Account:`, JSON.stringify({ name, token, power }));
...
const mergePetIds = async (headers, proxy) => {
    ...
    if (petIds.momPetIds.length < 1) {
        log.warn("you don't have any female pets");
        return;
    }
    ...
    while (moms.length > 0) {
        ...
        if (mom !== undefined && dad !== undefined) {
            log.info(`merge pets ${mom} and ${dad}`);
            await merge(headers, proxy, mom, dad);
            ...
            if (mom !== nextMom) {
                log.info(`merge pets ${mom} and ${nextMom}`);
                await merge(headers, proxy, mom, nextMom);
            ...
            log.warn("you don't have any couple”.");
            break;
        }
    }
};
...
async function startMission() {
    ...
    for (const user of users) {
        ...
        log.info("Fetching pet mom and dad can marge");
